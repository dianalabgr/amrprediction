...
# Get assembly and genus from Snakemake
assembly = os.path.basename(snakemake.input.assembly)  # Get the assembly file name
genus = snakemake.params.genus  # Get the genus from Snakemake parameters
reference_dir = snakemake.params.reference_dir  # Get the reference directory
...
# Get the reference genome based on the genus provided
ref_genome = reference_genomes.get(genus)
if not ref_genome:
    raise ValueError(f"Genus '{genus}' not found in reference genomes.")

# Set the directory name for the reference genome
dir_name = f"{reference_dir}{ref_genome}/"
